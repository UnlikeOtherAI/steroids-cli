...
*   **Global DB Reconciliation:** To guarantee this, the `steroids tasks reset` command must perform the following actions in the global database (`~/.steroids/steroids.db`):
    1.  **Locate Workstream:** Find the active or blocked `workstream` associated with the task's `section_id`.
    2.  **Revoke Lease:** Clear the `runner_id` and `lease_expires_at` on that workstream to mark it as available for a new runner.
