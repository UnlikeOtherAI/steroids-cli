...
## Provider CLI Reference Docs

Detailed per-provider reference documentation is maintained separately:

- **[Claude CLI Reference](../providers/claude.md)** — Stream-json parsing, session resume, prompt caching, token extraction
- **[Codex CLI Reference](../providers/codex.md)** — Stdin piping, thread-based sessions, `--json` mode
- **[Gemini CLI Reference](../providers/gemini.md)** — Flag ordering quirks, implicit caching, RESOURCE_EXHAUSTED handling
- **[Vibe CLI Reference](../providers/vibe.md)** — Agent loop internals, `-p` programmatic mode, filesystem session extraction, auto-context injection

These docs cover invocation patterns, model selection, output formats, session management, known issues, error patterns, and Steroids integration notes for each provider.
