...
export interface InvocationLogEntry {
  timestamp: string;
  role: 'orchestrator' | 'coder' | 'reviewer' | 'merge' | 'rebase_coder' | 'rebase_reviewer';
  provider: string;
  model: string;
  taskId?: string;
  duration: number;
...
export async function logInvocation(
  ...
  const canDbLog =
    ...
    (metadata.role === 'coder' || metadata.role === 'reviewer' || metadata.role === 'merge' || metadata.role === 'rebase_coder' || metadata.role === 'rebase_reviewer');
